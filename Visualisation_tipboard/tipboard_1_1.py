...
def prepare_for_just_value(data):
    ...
    data_prepared = data
    data_prepared = {'title': 'Shooting accuracy:', 'description': '', 'just-value': data_prepared}
...
def main():
    ...
    TILE_KEY = 'shooting_accuracy'
 
    c = Consumer({
       'bootstrap.servers': '86.119.35.55:9092',
       'group.id': 'test-consumer-group-shooting-accuracy',
       'default.topic.config': {
           'auto.offset.reset': 'largest'
       }
    })
 
    c.subscribe(['hrv_with_level_bpm_s'])
    ...
    while True:
       ...
       data_selected = data.get('HRV_LEVEL_S')
       print(data_selected)
